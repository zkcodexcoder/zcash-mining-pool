//! Owner-run, read-only testnet wallet collection-stage probe. No arguments are accepted.
...
//! This diagnostic does NOT validate full backing or authorize funding. The
//! original private input schema is retained for the same fixed owner wrapper.
use node_rpc::{ZcashRpcClient, zecd_funding::probe_testnet_funding};
...
impl Report {
    fn new() -> Self {
        Self { read_only:true, funding_authorization:false,
            passed:false, stage:"input", category:"input_invalid" }
    }
}
            ...
            .as_deref().and_then(parse_input)
        {
            if let Some(remaining) = TOTAL_LIMIT.checked_sub(started.elapsed()) {
                if tokio::time::timeout(remaining,probe(&input,&mut report)).await.is_err() {
                    report.passed = false;
                    report.category = "deadline_exceeded";
                }
            } else { report.category = "deadline_exceeded"; }
        }
    }
    // Fixed booleans plus static stage/category only; never financial authority.
    let output = serde_json::to_string(&report).unwrap_or_else(|_|String::from(
        "{\"read_only\":true,\"funding_authorization\":false,\"passed\":false,\"stage\":\"serialization\",\"category\":\"serialization_failed\"}"));
    println!("{output}");
...
mod tests {
    ...
    #[test]
    fn input_requires_original_network_loopback_pin_and_bounded_private_schema() {
    ...
    fn fixed_output_cannot_expose_input_or_financial_values() {
        ...
        for name in ["read_only","funding_authorization","passed"] { assert!(v[name].is_boolean()); }
        assert_eq!(v["read_only"],true);
        assert_eq!(v["funding_authorization"],false);
        assert_eq!(v["stage"],"input");
        assert_eq!(v["category"],"input_invalid");

...
mod tests {
    ...
    #[tokio::test]
    async fn authorize_passes_password_difficulty_to_the_validator() {
        let (event_tx, mut events) = mpsc::channel(8);
        let (server, _) = StratumServer::new(4, event_tx);
        let mut session = MinerSession::new("vardiff-session".into(), "de810000".into());
        session.subscribed = true;
        server.handle_request(&mut session, ClientRequest::Authorize {
            id: serde_json::json!(1), worker_name: "synthetic.worker".into(),
            worker_password: "d=5000".into(),
        }, "127.0.0.1:55555".parse().unwrap(), 3336).await;
        match events.try_recv().unwrap() {
            StratumEvent::WorkerConnected { password, .. } => assert_eq!(password, "d=5000"),
            _ => panic!("unexpected event"),
        }
    }

    #[tokio::test]
    async fn retarget_messages_reach_the_session_unchanged() {
        let server = server_with_ports();
        let (sender, mut messages) = mpsc::channel(8);
        server.session_senders.write().await.insert("vardiff-session".into(), sender);
        server.send_to_session("vardiff-session", ServerMessage::SetTarget { target: "ab".repeat(32) }).await;
        match messages.recv().await.unwrap() {
            ServerMessage::SetTarget { target } => assert_eq!(target, "ab".repeat(32)),
            _ => panic!("unexpected target message"),
        }
        server.send_to_session("vardiff-session", ServerMessage::SetDifficulty { difficulty: 5000.0 }).await;
        match messages.recv().await.unwrap() {
            ServerMessage::SetDifficulty { difficulty } => assert_eq!(difficulty, 5000.0),
